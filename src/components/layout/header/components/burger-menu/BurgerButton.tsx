...
const BurgerButton = ({ btnRef, showSidebar, setShowSidebar }: Props) => {
	const handleClick = () => {
		if (btnRef.current) {
			// btnRef.current.classList.toggle(`${styles.burger_btn_open}`)
			// btnRef.current.setAttribute(
			// 	'aria-expanded',
			// 	`${btnRef.current.classList.contains(`${styles.opened}`)}`
			// )
